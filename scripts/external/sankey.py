...
def check_data_matches_labels(labels, data, side):
    if len(labels) > 0:
        ...
        if labels != data:
            ...
            if len(data) < 20:
                msg += "Data\n" + "\n".join(data)
            raise LabelMismatch('{0}-side labels and data do not match.\n\n{1}'.format(side, msg))
...
def sankey(left, right, leftWeight=None, rightWeight=None, colorDict=None,
           ...
           fontsize=14, 
           left_category_name='', right_category_name='', 
           figure_name=None,closePlot=False):
    '''
    ...
    # for labels at the top
    y_pos = topEdge + 20

    y_pos = -40
    plt.text(0, y_pos, left_category_name, fontsize=14, ha='center')
    plt.text(xMax, y_pos, right_category_name, fontsize=14, ha='center')
